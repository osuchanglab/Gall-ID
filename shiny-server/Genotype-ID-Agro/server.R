...
perez_defined_genes <- c("recA", "glnA", "dnaK", "rpoB", "gyrB", "truA", "thrA")
test_defined_genes <- c("CelA", "DnaA", "GyrB")
agrobacterium_16S_defined_genes <- c("16S")
alexandre_dnaJ_defined_genes <- c("dnaJ")
alexandre_MLSA_defined_genes <- c("atpD", "gapA", "gyrB", "recA", "rplB")
  ...
  alin <- reactive({
    if (gsub("\\s", "", input$fasta) == ""){
      ...
      if (startsWith(input$fasta,">") == TRUE){
		...
		if (input_datasetname == "perez-yepez") {
			defined_genes <- perez_defined_genes
		} else if (input_datasetname == "alexandre-MLSA") {
			defined_genes <- alexandre_MLSA_defined_genes
		} else if (input_datasetname == "alexandre-dnaJ") {
			defined_genes <- alexandre_dnaJ_defined_genes
        } else if (input_datasetname == "Agrobacterium-16S") {
			defined_genes <- agrobacterium_16S_defined_genes
        } else {
